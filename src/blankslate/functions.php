...
add_action( 'wp_enqueue_scripts', 'custom_stylesheets_setup' );
function custom_stylesheets_setup() {
	wp_enqueue_style('google-fonts', 'https://fonts.googleapis.com/css?family=Open+Sans');
}

add_action( 'wp_enqueue_scripts', 'custom_scripts_setup' );
function custom_scripts_setup() {
	wp_enqueue_script('grabjet', get_stylesheet_directory_uri() . '/js/scripts.js');
}